    ...
    setCarrinho((prev) => {
      ...
      if (itemExistente) {
        return prev.map((item) =>
          item.id === produtoSelecionado.id ? { ...item, quantidade: item.quantidade + quantidade } : item
        );
      } else {
      ...
      <div id="carouselExample" className="carousel slide" data-bs-ride="carousel">
        <div className="carousel-inner">
          <div className="carousel-item active">
            <img src="https://dummyimage.com/800x400/000/fff" className="d-block w-100" alt="Banner 1"/>
          </div>
          <div className="carousel-item">
            <img src="https://dummyimage.com/800x400/111/eee" className="d-block w-100" alt="Banner 2"/>
          </div>
        </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                {carrinho.length === 0 ? <p>O carrinho está vazio</p> : (
                  <ul>
                    {carrinho.map((item) => (
                      <li key={item.id}>{item.nome} - {item.quantidade}x R${item.preco.toFixed(2)}</li>
                    ))}
                  </ul>
                )}
